...
// ── MOBILE MENU ──
function toggleMob() {
  const isOpen = document.getElementById('mmenu').classList.toggle('open');
  const btn = document.querySelector('.ham');
  if (btn) btn.setAttribute('aria-expanded', String(isOpen));
}
function toggleSub(id) {
  const isOpen = document.getElementById(id).classList.toggle('open');
...
function faq(el) {
  ...
  item.parentElement.querySelectorAll('.fi').forEach(i => {
    i.classList.remove('open');
    const trigger = i.querySelector('.fi-q');
